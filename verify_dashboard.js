...
async function main() {
  ...
  if (d.knights?.details?.length > 0) {
    d.knights.details.slice(0, 3).forEach((k, i) => {
  ...
  wl.slice(0, 2).forEach((w, i) => {
    console.log(`\n运单 ${i + 1}: ${w.order_no}, status=${w.status}`);
    console.log(`  寄件人: ${w.sender_name} -> 收件人: ${w.receiver_name}`);
    console.log(`  品类: ${w.category}, 保价: ${w.dispatch?.insurance_label} (¥${(w.dispatch?.insurance_value || 0).toFixed(2)})`);
