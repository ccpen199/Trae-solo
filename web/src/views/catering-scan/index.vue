<template>
  <div class="scan-container">
    <div class="page-header">
      <h2 class="page-title">餐饮消费</h2>
      <p class="page-desc">扫描条码进行餐饮消费核验</p>
    </div>
          ...
          <div v-if="scanResult" class="scan-result" :class="scanResultType">
            ...
            <div v-if="scanResultType === 'success' && scanResult.data" class="result-details">
              ...
              <div class="detail-item">
                ...
                <span class="value">{{ formatTime(scanResult.data.consumedAt) }}</span>
              </div>
            </div>
          </div>
    ...
    <el-row class="mt-20">
      <el-col :span="24">
        <div class="card-container">
          <h3 class="area-title">今日消费记录 ({{ todayCount }} 条)</h3>
          <el-table :data="recentRecords" style="width: 100%" v-loading="loading">
            ...
            <el-table-column prop="consumedAt" label="时间" width="180">
              <template #default="scope">
                {{ formatTime(scope.row.consumedAt) }}
              </template>
            </el-table-column>
          </el-table>
        </div>
...
const scanResultType = computed(() => {
  ...
  if (scanResult.value.code === 'NO_CATERING_COUNT' || scanResult.value.code === 'DUPLICATE_SCAN') return 'warning'
  return 'error'
})
...
async function fetchRecentRecords() {
  loading.value = true
  try {
    const res = await request.get('/api/catering/recent', { params: { limit: 20 } })
    if (res.success) {
...
async function handleScan() {
  ...
  try {
    ...
    const res = await request.post('/api/catering/scan', data)
    ...
    scanResult.value = {
      success: false,
      message: error.response?.data?.message || '消费失败，请稍后重试'
    }
  } finally {
