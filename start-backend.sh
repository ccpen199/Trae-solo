...
if command -v screen >/dev/null 2>&1; then
  screen -dmS may-89064-backend bash -lc 'cd "$1"; log="$2"; shift 2; exec "$@" > "$log" 2>&1' _ "$BACKEND_DIR" "$LOG_FILE" env PATH="$(dirname "$NODE_BIN"):$PATH" FRONTEND_PORT="$FRONTEND_PORT" BACKEND_PORT="$BACKEND_PORT" DATABASE_URL="${DATABASE_URL:-./data/app.sqlite}" "$NODE_BIN" src/server.js
else
  (cd "$BACKEND_DIR" && nohup env PATH="$(dirname "$NODE_BIN"):$PATH" FRONTEND_PORT="$FRONTEND_PORT" BACKEND_PORT="$BACKEND_PORT" DATABASE_URL="${DATABASE_URL:-./data/app.sqlite}" "$NODE_BIN" src/server.js > "$LOG_FILE" 2>&1 < /dev/null &)
fi

for _ in {1..40}; do
  pid="$(port_pid "$BACKEND_PORT")"
  if [ -n "$pid" ] && pid_in_project "$pid" && curl -sS --max-time 1 "http://127.0.0.1:$BACKEND_PORT/api/health" >/dev/null 2>&1; then
    echo "$pid" > "$PID_FILE"
