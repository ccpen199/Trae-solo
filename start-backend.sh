...
SESSION_NAME="${PROJECT_NAME:-may-89116}-backend"
if [ -n "$TMUX_BIN" ]; then
  ...
  "$TMUX_BIN" new-session -d -s "$SESSION_NAME" -c "$PROJECT_DIR" \
    "echo \$\$ | tee .backend.pid backend.pid >/dev/null; exec env HOST='${HOST:-127.0.0.1}' BACKEND_PORT='$BACKEND_PORT' NODE_ENV='${NODE_ENV:-development}' PATH='$PATH' '$NODE_BIN' --import tsx/esm api/server.ts >> backend.log 2>&1"
else
  nohup "$NODE_BIN" --import tsx/esm api/server.ts </dev/null > backend.log 2>&1 &
  echo $! > .backend.pid
  echo $! > backend.pid
