...
SESSION_NAME="${PROJECT_NAME:-may-89116}-frontend"
if [ -n "$TMUX_BIN" ]; then
  ...
  "$TMUX_BIN" new-session -d -s "$SESSION_NAME" -c "$PROJECT_DIR" \
    "echo \$\$ | tee .frontend.pid frontend.pid >/dev/null; exec env HOST='${HOST:-127.0.0.1}' FRONTEND_PORT='$FRONTEND_PORT' BACKEND_PORT='$BACKEND_PORT' NODE_ENV='${NODE_ENV:-development}' PATH='$PATH' '$NODE_BIN' node_modules/vite/bin/vite.js --host 127.0.0.1 --port '$FRONTEND_PORT' --strictPort >> frontend.log 2>&1"
else
