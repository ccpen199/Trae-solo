...
ENV_FILE="$ROOT/.env"
BASE_PORT="${FRONTEND_PORT:-48783}"
PORT="$BASE_PORT"
BACKEND_HEALTH_URL="http://127.0.0.1:${BACKEND_PORT:-58783}/api/health"
...
choose_frontend_port() {
  ...
  for offset in $(seq 0 10); do
    ...
    if [ "$cwd" = "$ROOT/frontend" ]; then
      ...
      if [[ "$command" == *"/vite/bin/vite.js"* || "$command" == *"/.bin/vite"* || "$command" == *" vite "* ]]; then
