...
stop_owned_port "$BACKEND_PORT"
: > backend.log

SESSION_NAME="${PROJECT_NAME:-may-89124}-backend"
if [ -n "$TMUX_BIN" ]; then
  ...
  "$TMUX_BIN" new-session -d -s "$SESSION_NAME" -c "$PROJECT_DIR" \
    "echo \$\$ > backend.pid; exec env HOST='$HOST' BACKEND_PORT='$BACKEND_PORT' NODE_ENV='$NODE_ENV' '$NODE_BIN' api/server.js >> backend.log 2>&1"
else
  nohup env \
    HOST="$HOST" \
