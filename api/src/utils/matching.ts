    ...
    if (scheduleDate >= start && scheduleDate <= end) {
      if (schedule.status === 'booked' || schedule.status === 'unavailable') {
        conflicts.push({
          ...
          description: `艺术家在 ${new Date(schedule.date).toDateString()} 已有安排: ${schedule.description || '已预约'}`,
        });
      }
    }
  }
...
export function filterArtists(
  ...
  return artists.filter((artist) => {
    if (filters.gender && artist.gender !== filters.gender) return false;
    if (filters.ageMin && artist.age < filters.ageMin) return false;
    if (filters.ageMax && artist.age > filters.ageMax) return false;
    if (filters.heightMin && artist.height < filters.heightMin) return false;
    if (filters.heightMax && artist.height > filters.heightMax) return false;
    if (filters.location && !artist.location.includes(filters.location)) return false;
    if (filters.contractStatus && artist.contractStatus !== filters.contractStatus) return false;

    if (filters.skills && filters.skills.length > 0) {
      const hasSkill = filters.skills.some((s) => artist.skills.includes(s));
      if (!hasSkill) return false;
    }

    if (filters.languages && filters.languages.length > 0) {
      const hasLang = filters.languages.some((l) => artist.languages.includes(l));
      if (!hasLang) return false;
    }

    if (filters.tags && filters.tags.length > 0) {
      const artistTags = artist.tags.map((t) => t.tag);
      const hasTag = filters.tags.some((t) => artistTags.includes(t));
