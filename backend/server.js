...
function projectServices() {
  if (projectName === 'may-89094') {
    return [
      ['求职者智能建档与简历OCR', '求职者服务', '山东人才档案中心', 0, 4.9, '可建档', '解析学历、技能、经验和期望薪资标签，支持简历OCR自动补全。'],
      ['山东16地市岗位智能匹配', '岗位匹配', '区域人才算法中心', 0, 4.9, '推荐中', '按济南、青岛、烟台等16地市、LBS距离、经验年限、技能栈和期望薪资推荐岗位。'],
      ['企业资质强审核', '企业审核', '资质风控台', 0, 4.8, '审核中', '核验营业执照、社保缴纳数据和用工黑名单，拦截异常招聘主体。'],
      ['面试预约与录用意向', '面试协同', '招聘流程工作台', 0, 4.7, '可预约', '支持在线预约、视频面试、面试评价模板和录用意向一键发送。'],
      ['人才供需热力与虚假职位拦截', '后台管理', '招聘监管驾驶舱', 0, 4.8, '监测中', '展示区域人才供需热力图、简历转化率、面试到场率、Offer接受率和虚假职位实时拦截。'],
      ['高校合作培训报名', '本地服务', '校招就业协同中心', 0, 4.6, '推进中', '跟踪高校合作看板、专业匹配度、校招进展、留存率和技校培训报名。'],
    ];
  }
...
function initDb() {
  ...
  const seedServices = projectServices();
  const hasProjectSeed = db.prepare('SELECT COUNT(*) AS count FROM services WHERE title = ?').get(seedServices[0][0]).count > 0;
  if (serviceCount === 0 || !hasProjectSeed) {
    const insert = db.prepare(`
      INSERT INTO services (title, category, provider, price, rating, status, description)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    seedServices.forEach((item) => insert.run(...item));
  }
...
function adminSummary() {
  ...
  if (projectName === 'may-89094') {
    summary.modules = ['登录注册', '山东16地市检索', '简历OCR建档', 'LBS岗位匹配', '面试协同', '后台监管'];
    summary.talentHeatmap = [
      { city: '济南', industry: '智能制造', jobs: 186, talents: 142, gap: 44 },
      { city: '青岛', industry: '现代服务', jobs: 214, talents: 260, gap: -46 },
      { city: '烟台', industry: '装备制造', jobs: 128, talents: 91, gap: 37 },
      { city: '临沂', industry: '物流供应链', jobs: 97, talents: 73, gap: 24 },
    ];
    summary.recruitmentMetrics = {
      resumeConversion: 62,
      interviewAttendance: 81,
      offerAcceptance: 57,
      fakeJobBlocked: 18,
    };
    summary.campusCooperation = [
      { school: '山东大学', progress: '春招复盘', majorMatch: 86, retention: 74 },
      { school: '青岛职业技术学院', progress: '蓝领技能专场', majorMatch: 91, retention: 69 },
      { school: '烟台工程职业技术学院', progress: '校企实训', majorMatch: 78, retention: 71 },
    ];
    summary.auditChecks = [
      { item: '营业执照核验', status: '通过', result: '统一社会信用代码一致' },
      { item: '社保缴纳比对', status: '通过', result: '近6个月缴费记录正常' },
      { item: '用工黑名单筛查', status: '通过', result: '未命中欠薪与劳动争议黑名单' },
      { item: '虚假职位拦截模型', status: '拦截', result: 'IP聚集与联系方式异常拦截18条' },
    ];
  }
  return summary;
}
...
async function handle(req, res) {
  ...
  if (req.method === 'POST' && ['/api/orders', '/api/purchase', '/api/buy'].includes(route)) {
    ...
    `).run(
      serviceId,
      String(body.customer || '演示用户'),
      String(body.phone || '13800138000'),
      Number(body.amount || item.price),
      projectName === 'may-89094' ? '匹配流程已生成' : '已提交',
      String(body.note || (projectName === 'may-89094' ? '生成简历建档、LBS岗位匹配、企业资质核验和面试预约流程' : '网页提交的购买申请')),
      now
    );
    db.prepare('INSERT INTO audit_logs (actor, action, created_at) VALUES (?, ?, ?)').run('user', projectName === 'may-89094' ? `提交岗位匹配流程 #${result.lastInsertRowid}` : `提交订单 #${result.lastInsertRowid}`, now);
