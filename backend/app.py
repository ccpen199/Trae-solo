...
BACKEND_PORT = int(os.environ.get("BACKEND_PORT", "59091"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in {"/api/categories", "/api/service-categories"}:
            ...
            return
        if path in {"/api/services", "/api/products", "/api/tasks", "/api/search"}:
        ...
        if path in {"/api/submissions", "/api/requests"}:
            ...
            return
        if path in {"/api/profile", "/api/user/profile"}:
            ...
            return
        if path in {"/api/admin/summary", "/api/admin/dashboard"}:
            summary = {
