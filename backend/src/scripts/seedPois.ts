...
const grids = db.prepare('SELECT code, lat, lng FROM grids LIMIT 32').all() as { code: string; lat: number; lng: number }[];
const providers = db.prepare('SELECT id FROM providers LIMIT 32').all() as { id: number }[];
...
console.log('\nPOI类型分布:');
types.forEach((t: any) => console.log(`  ${t.type}: ${t.cnt}个`));
