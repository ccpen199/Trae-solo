...
@Entity('category_permissions')
@Index(['categoryId', 'roleId'], { unique: true, where: "role_id IS NOT NULL" })
@Index(['categoryId', 'userId'], { unique: true, where: "user_id IS NOT NULL" })
export class CategoryPermission {
  ...
  @Column({ type: 'boolean', default: false, name: 'inherit_from_parent' })
