...
const encryptionService_1 = __importDefault(require("../src/services/encryptionService"));
const enums_1 = require("../src/types/enums");
const prisma = new prisma_1.PrismaClient();
async function main() {
    console.log("开始播种数据...");
    const passwordHash = await bcryptjs_1.default.hash("123456", 12);
    const idCardEncrypted = encryptionService_1.default.encrypt("110101199001011234");
    const hisApiConfigEncrypted = encryptionService_1.default.encryptObject({
        url: "https://hospital-api.example.com",
        apiKey: "test-api-key",
        secret: "test-secret",
    });
    const apiConfigEncrypted = encryptionService_1.default.encryptObject({
    ...
    const testUser = await prisma.user.create({
        data: {
            phone: "13900139000",
            name: "张三",
            idCardEncrypted: encryptionService_1.default.encrypt("110101198505051234"),
    ...
    const institutions = [
        {
            name: "北京协和医院",
            type: enums_1.InstitutionType.HOSPITAL,
            city: "北京",
            address: "北京市东城区帅府园1号",
            phone: "010-69156114",
            level: "三级甲等",
    ...
    for (const inst of institutions) {
        const institution = await prisma.institution.create({
            data: {
                ...inst,
                status: enums_1.InstitutionStatus.APPROVED,
                approvedAt: new Date(),
    ...
    console.log("创建体检套餐...");
    const packageTypes = [
        { type: enums_1.PackageType.GENERAL, namePrefix: "基础体检套餐", price: 599, desc: "适合常规健康检查的基础套餐" },
        { type: enums_1.PackageType.CARDIOVASCULAR, namePrefix: "心血管专项套餐", price: 1299, desc: "针对心血管健康的专项检查" },
        { type: enums_1.PackageType.CANCER_SCREENING, namePrefix: "肿瘤筛查套餐", price: 1999, desc: "针对常见癌症的早期筛查套餐" },
        { type: enums_1.PackageType.WOMEN_HEALTH, namePrefix: "女性健康套餐", price: 899, desc: "针对女性健康的专项体检套餐" },
    ];
        ...
        for (let j = 0; j < packageTypes.length; j++) {
            ...
            const healthPackage = await prisma.healthPackage.create({
                data: {
                    ...
                    status: enums_1.PackageStatus.PUBLISHED,
                },
            });
    ...
    console.log("创建保险产品...");
    const productTypes = [
        { type: enums_1.InsuranceProductType.CRITICAL_ILLNESS, name: "重大疾病保险", premium: 3500 },
        { type: enums_1.InsuranceProductType.HOSPITALIZATION, name: "百万医疗险", premium: 299 },
        { type: enums_1.InsuranceProductType.ACCIDENT, name: "意外伤害保险", premium: 199 },
    ];
            ...
            const product = await prisma.insuranceProduct.create({
                data: {
                    ...
                    coverage: JSON.stringify({
                        coverageAmount: prod.type === enums_1.InsuranceProductType.CRITICAL_ILLNESS ? 500000 : prod.type === enums_1.InsuranceProductType.HOSPITALIZATION ? 4000000 : 100000,
                        coveragePeriod: prod.type === enums_1.InsuranceProductType.CRITICAL_ILLNESS ? "LIFETIME" : "ONE_YEAR",
                        waitingPeriod: prod.type === enums_1.InsuranceProductType.CRITICAL_ILLNESS ? 90 : 30,
                    }),
                    ...
                    underwritingRules: JSON.stringify({
                        ...
                        medicalExamRequired: prod.type === enums_1.InsuranceProductType.CRITICAL_ILLNESS,
                    }),
    ...
    const checkupPersonEncrypted = encryptionService_1.default.encryptObject({
        name: "张三",
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const booking = await prisma.booking.create({
            data: {
                ...
                status: i === 0 ? enums_1.BookingStatus.COMPLETED : i === 1 ? enums_1.BookingStatus.CONFIRMED : enums_1.BookingStatus.PENDING,
    ...
    const applicantEncrypted = encryptionService_1.default.encryptObject({
        name: "张三",
        idCard: "110101198505051234",
        phone: "13900139000",
        gender: "MALE",
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const order = await prisma.insuranceOrder.create({
            data: {
                ...
                status: i === 0 ? enums_1.InsuranceOrderStatus.ACTIVE : i === 1 ? enums_1.InsuranceOrderStatus.APPROVED : enums_1.InsuranceOrderStatus.PENDING_ASSESSMENT,
    ...
    if (bookings.length > 0) {
        ...
        const indicators = [
            { category: "血常规", name: "白细胞计数", value: "6.8", unit: "×10^9/L", referenceRange: "4.0-10.0", status: enums_1.IndicatorStatus.NORMAL },
            { category: "血常规", name: "红细胞计数", value: "4.8", unit: "×10^12/L", referenceRange: "4.0-5.5", status: enums_1.IndicatorStatus.NORMAL },
            { category: "血常规", name: "血红蛋白", value: "145", unit: "g/L", referenceRange: "120-160", status: enums_1.IndicatorStatus.NORMAL },
            { category: "肝功能", name: "谷丙转氨酶(ALT)", value: "45", unit: "U/L", referenceRange: "0-40", status: enums_1.IndicatorStatus.ABNORMAL },
            { category: "血脂", name: "总胆固醇", value: "5.8", unit: "mmol/L", referenceRange: "0-5.2", status: enums_1.IndicatorStatus.ABNORMAL },
            { category: "血糖", name: "空腹血糖", value: "6.5", unit: "mmol/L", referenceRange: "3.9-6.1", status: enums_1.IndicatorStatus.ABNORMAL },
        ];
        const abnormalIndicators = indicators
            .filter((i) => i.status !== enums_1.IndicatorStatus.NORMAL)
            .map((i) => ({
            name: i.name,
            value: i.value,
            referenceRange: i.referenceRange,
            level: enums_1.AbnormalLevel.MILD,
        ...
        const archive = await prisma.$transaction(async (tx) => {
            const newArchive = await tx.healthArchive.create({
                data: {
                    ...
                    ocrStatus: enums_1.OcrStatus.COMPLETED,
                    structuredData: JSON.stringify({
                        reportType: "体检报告",
                        analysisDate: new Date().toISOString(),
                        summary: {
                            totalIndicators: indicators.length,
                            normalCount: indicators.filter((i) => i.status === enums_1.IndicatorStatus.NORMAL).length,
                            abnormalCount: indicators.filter((i) => i.status !== enums_1.IndicatorStatus.NORMAL).length,
                        },
                        recommendations: [
                            "建议控制饮食，减少高脂肪、高糖食物摄入",
