...
function authMiddleware(requiredRoles = []) {
  return async (req, res, next) => {
    try {
      ...
      const user = await db.get('SELECT id, username, role, status, parent_id FROM users WHERE id = ?', payload.userId);
      if (!user) {
...
async function validateDevicePermission(userId, deviceId, requiredPermission = 'view') {
...
function riskControlMiddleware() {
  return async (req, res, next) => {
    try {
      ...
      `, username, oneHourAgo);
      const failCount = failRow ? failRow.count : 0;
