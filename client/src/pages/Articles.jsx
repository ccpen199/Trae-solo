...
import { Flame, Clock, Sparkles, ChevronDown, FileText } from 'lucide-react';
          ...
          {showCategoryDropdown && (
            <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
              <button
                ...
                className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 ${!categoryId ? 'text-blue-600 font-medium' : 'text-gray-700'}`}
              >
                全部分类
              </button>
              {categories.map((cat) => (
                <button
                  ...
                  className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 ${categoryId === String(cat.id) ? 'text-blue-600 font-medium' : 'text-gray-700'}`}
                >
                  {cat.name}
                </button>
              ))}
          ...
          {sortOptions.map((option) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                sort === option.value ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
              {option.icon}
              {option.label}
