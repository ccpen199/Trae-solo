    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="card p-6 mb-8">
        ...
        {loading ? (
          ...
          <div>
            <div className="flex items-end justify-around gap-2 mb-8 h-48 px-4">
              {trends.map((trend) => (
                <div key={trend.category} className="flex flex-col items-center flex-1 max-w-[100px]">
                  <div className="text-xs font-semibold text-gray-700 mb-1">{trend.taskCount}</div>
                  <div 
                    className="w-full bg-gradient-to-t from-blue-500 to-purple-500 rounded-t-lg transition-all duration-700 hover:from-blue-600 hover:to-purple-600"
                    style={{ height: `${(trend.taskCount / maxTaskCount) * 150}px`, minHeight: '20px' }}
                    title={`${categoryLabels[trend.category] || trend.category}: ${trend.taskCount}个任务`}
                  ></div>
                  <div className="text-xs text-gray-500 mt-2 text-center truncate w-full">
                    {categoryLabels[trend.category]?.replace('服务', '') || trend.category}
                  </div>
                </div>
              ))}
            </div>
                ...
                {trends.map((trend) => (
                  <div key={trend.category} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      <h3 className="font-semibold text-gray-900 text-sm">{categoryLabels[trend.category] || trend.category}</h3>
                      <span className="badge bg-primary-50 text-primary-700 flex items-center text-xs">
