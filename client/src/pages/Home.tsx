...
import { Search, Briefcase, Palette, Code, FileText, Megaphone, Home as HomeIcon, TrendingUp, Users, Award, ChevronRight } from 'lucide-react'
import api from '../api'
import { Task, Skill } from '../types'

const categories = [
  { name: '设计服务', icon: Palette, color: 'from-pink-500 to-rose-500', count: 12580 },
  { name: '开发服务', icon: Code, color: 'from-blue-500 to-indigo-500', count: 8942 },
  { name: '文案撰写', icon: FileText, color: 'from-amber-500 to-orange-500', count: 6523 },
  { name: '营销推广', icon: Megaphone, color: 'from-green-500 to-emerald-500', count: 5120 },
  { name: '装修设计', icon: HomeIcon, color: 'from-purple-500 to-violet-500', count: 3218 },
  { name: '视频制作', icon: TrendingUp, color: 'from-red-500 to-pink-500', count: 2856 },
]
...
export default function Home() {
  ...
  const stats = [
    { label: '累计任务数', value: '38,650+', icon: Briefcase },
    { label: '认证服务商', value: '12,380+', icon: Users },
    { label: '累计成交额', value: '¥5.2亿', icon: Award },
    { label: '好评率', value: '98.6%', icon: TrendingUp },
  ]
          ...
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-6 text-sm">
              ...
              {['UI设计', '微信小程序', 'Logo设计', '短视频脚本', '网站开发'].map((tag) => (
                <span key={tag} className="px-3 py-1 bg-white/10 rounded-full hover:bg-white/20 cursor-pointer transition-colors">
                  {tag}
                </span>
              ))}
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map((cat) => (
            <Link
              key={cat.name}
              to={`/tasks?category=${cat.name === '设计服务' ? 'DESIGN' : cat.name === '开发服务' ? 'DEVELOPMENT' : cat.name === '文案撰写' ? 'COPYWRITING' : cat.name === '营销推广' ? 'MARKETING' : cat.name === '装修设计' ? 'DECORATION' : 'VIDEO'}`}
            ...
            >
              ...
              <p className="text-xs text-gray-500">{cat.count.toLocaleString()} 个任务</p>
            </Link>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-500 mt-2">查看各领域专业技能</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {!loading && skills.map((skill) => (
              ...
              >
                {skill.name}
                <span className="ml-2 text-xs text-gray-400">{skill.demandCount}</span>
              </Link>
            ))}
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <span className="badge bg-primary-50 text-primary-700">
                    {task.category === 'DESIGN' ? '设计' : task.category === 'DEVELOPMENT' ? '开发' : task.category === 'COPYWRITING' ? '文案' : task.category}
                  </span>
