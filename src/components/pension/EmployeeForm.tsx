import { User, CalendarDays, Clock, TrendingUp, Wallet, Landmark, FileText, Award } from 'lucide-react'
...
export default function EmployeeForm({ form, onChange }: EmployeeFormProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pt-3 border-t border-gray-100">
        <p className="text-xs text-gray-500 mb-3">以下为补充参数（可选）</p>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
              <FileText size={14} /> 视同缴费年限
            </label>
            <input
              type="number"
              min={0}
              max={30}
              value={form.deemedPaymentYears}
              onChange={(e) => update('deemedPaymentYears', Number(e.target.value) || 0)}
              className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
              placeholder="0"
            />
          </div>
          <div className="space-y-1">
            <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
              <Award size={14} /> 过渡性养老金（元）
            </label>
            <input
              type="number"
              min={0}
              value={form.transitionalPension}
              onChange={(e) => update('transitionalPension', Number(e.target.value) || 0)}
              className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
              placeholder="0"
            />
          </div>
