import { User, CalendarDays, Clock, Wallet, Landmark, Gift } from 'lucide-react'
import type { ResidentFormData } from './types'
import { residentPaymentGrades, residentReceiveAges } from './types'

interface ResidentFormProps {
  form: ResidentFormData
  onChange: (data: ResidentFormData) => void
}

export default function ResidentForm({ form, onChange }: ResidentFormProps) {
  const update = <K extends keyof ResidentFormData>(key: K, value: ResidentFormData[K]) => {
    onChange({ ...form, [key]: value })
  }

  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <User size={15} /> 性别
        </label>
        <div className="flex gap-3">
          {(['male', 'female'] as const).map((g) => (
            <label
              key={g}
              className="flex items-center gap-2 cursor-pointer px-4 py-2 rounded-lg border text-sm transition-colors"
              style={{
                borderColor: form.gender === g ? '#165DFF' : '#E5E6EB',
                backgroundColor: form.gender === g ? '#E8F0FF' : '#fff',
                color: form.gender === g ? '#165DFF' : '#4E5969',
              }}
            >
              <input
                type="radio"
                name="gender"
                value={g}
                checked={form.gender === g}
                onChange={() => update('gender', g)}
                className="accent-[#165DFF]"
              />
              {g === 'male' ? '男' : '女'}
            </label>
          ))}
        </div>
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <CalendarDays size={15} /> 出生年月
        </label>
        <input
          type="month"
          value={form.birthMonth}
          onChange={(e) => update('birthMonth', e.target.value)}
          className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
        />
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <Clock size={15} /> 领取年龄
        </label>
        <select
          value={form.receiveAge}
          onChange={(e) => update('receiveAge', Number(e.target.value))}
          className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 bg-white border-gray-200"
        >
          {residentReceiveAges.map((age) => (
            <option key={age} value={age}>{age}岁</option>
          ))}
        </select>
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <Clock size={15} /> 缴费年限（年）
        </label>
        <div className="flex items-center gap-3">
          <input
            type="range"
            min={1}
            max={30}
            value={form.paymentYears}
            onChange={(e) => update('paymentYears', Number(e.target.value))}
            className="flex-1 accent-primary"
          />
          <input
            type="number"
            min={1}
            max={30}
            value={form.paymentYears}
            onChange={(e) => {
              const v = Math.min(30, Math.max(1, Number(e.target.value) || 1))
              update('paymentYears', v)
            }}
            className="w-16 px-2 py-1.5 rounded-lg border text-sm text-center focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
          />
        </div>
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <Wallet size={15} /> 年缴费档次（元）
        </label>
        <div className="grid grid-cols-3 gap-2">
          {residentPaymentGrades.map((grade) => (
            <button
              key={grade}
              onClick={() => update('annualPaymentGrade', grade)}
              className={`py-2 px-2 rounded-lg text-sm font-medium transition-colors border ${
                form.annualPaymentGrade === grade
                  ? 'bg-primary text-white border-primary'
                  : 'bg-white text-gray-700 border-gray-200 hover:border-primary hover:text-primary'
              }`}
            >
              ¥{grade}
            </button>
          ))}
        </div>
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <Gift size={15} /> 政府补贴（元/年）
        </label>
        <input
          type="number"
          min={0}
          value={form.governmentSubsidy}
          onChange={(e) => update('governmentSubsidy', Number(e.target.value) || 0)}
          className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
          placeholder="请输入政府补贴"
        />
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <Landmark size={15} /> 个人账户余额（元）
        </label>
        <input
          type="number"
          value={form.personalAccountBalance || ''}
          onChange={(e) => update('personalAccountBalance', Number(e.target.value) || 0)}
          className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
          placeholder="请输入个人账户余额"
        />
      </div>

      <div className="space-y-1">
        <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700">
          <Landmark size={15} /> 基础养老金标准（元/月）
        </label>
        <input
          type="number"
          min={0}
          value={form.basicPensionStandard}
          onChange={(e) => update('basicPensionStandard', Number(e.target.value) || 0)}
          className="w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 border-gray-200"
          placeholder="请输入当地基础养老金标准"
        />
      </div>
    </div>
  )
}
