...
export const solarToLunar = (date: Date): LunarDate => {
  ...
  for (; year < 2100 && offset > 0; year++) {
    yearDays = lYearDays(year, lunarInfo);
    if (offset < yearDays) break;
    offset -= yearDays;
  }

  const leapMonth = leapMonthOfYear(year, lunarInfo);
  let isLeap = false;
  let month = 1;
  ...
  for (; month < 13 && offset > 0; month++) {
    if (leapMonth > 0 && month === leapMonth + 1 && !isLeap) {
      --month;
      isLeap = true;
      monthDays = leapDays(year, lunarInfo);
    } else {
      monthDays = monthDaysOfYear(year, month, lunarInfo);
    }
...
const lYearDays = (year: number, lunarInfo: number[]): number => {
  let sum = 348;
...
const leapMonthOfYear = (year: number, lunarInfo: number[]): number => {
  return lunarInfo[year - 1900] & 0xf;
};

const leapDays = (year: number, lunarInfo: number[]): number => {
  if (leapMonthOfYear(year, lunarInfo)) {
    return lunarInfo[year - 1900] & 0x10000 ? 30 : 29;
  }
  return 0;
};

const monthDaysOfYear = (
  year: number,
  month: number,
  lunarInfo: number[]
): number => {
...
const getTodayYiJi = (date: Date): { yi: YiJiItem[]; ji: YiJiItem[] } => {
...
export const getCurrentSolarTerm = (date: Date): SolarTerm | undefined => {
  const month = date.getMonth() + 1;
  const day = date.getDate();
  const currentMMDD = `${month.toString().padStart(2, "0")}-${day.toString().padStart(2, "0")}`;

  let current: SolarTerm | undefined;
  let next: SolarTerm | undefined;
...
export const getNextSolarTerm = (date: Date): SolarTerm | undefined => {
  const month = date.getMonth() + 1;
  const day = date.getDate();
  const currentMMDD = `${month.toString().padStart(2, "0")}-${day.toString().padStart(2, "0")}`;

  for (let i = 0; i < solarTerms.length; i++) {
    if (solarTerms[i].date > currentMMDD) {
      return solarTerms[i];
    }
  }
  return solarTerms[0];
...
export const getMemorialDays = (): MemorialDay[] => {
...
export const getCalendarData = (date: Date = new Date()): CalendarData => {
  const lunar = solarToLunar(date);
  const { yi, ji } = getTodayYiJi(date);
  const currentSolarTerm = getCurrentSolarTerm(date);
  const nextSolarTerm = getNextSolarTerm(date);
  const memorialDays = getMemorialDays();
