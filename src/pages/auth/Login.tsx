...
const Login = () => {
  ...
  const fillTestAccount = (account: string, password: string, role: UserRole) => {
    ...
    toast.info("已填充测试账号", `账号：${account}，角色：${roleNames[role]}`)
  }
              ...
              <form onSubmit={handleLoginSubmit(onLogin)} className="space-y-5">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-midnight-200">选择角色</span>
                    <button
                      type="button"
                  ...
                  {loginErrors.role && (
                    <p className="text-sm text-red-400">{loginErrors.role.message}</p>
                  )}
            ...
            <TabsContent value="register">
              <form onSubmit={handleRegisterSubmit(onRegister)} className="space-y-5">
                <div className="space-y-3">
                  <span className="text-sm font-medium text-midnight-200">选择角色</span>
                  {registerErrors.role && (
                    <p className="text-sm text-red-400">{registerErrors.role.message}</p>
                  )}
                  <Controller
                    name="role"
                    control={registerControl}
