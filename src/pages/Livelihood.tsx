...
import { mockSubsidies, mockMedicalInsurance } from "@/data/mock";
import StatusBadge from "@/components/StatusBadge";
import AnimatedNumber from "@/components/AnimatedNumber";
import { HeartPulse, HandCoins, Baby, ShieldCheck, CheckCircle2, Clock, ArrowRight, QrCode } from "lucide-react";
...
export default function Livelihood() {
  ...
  const insurance = mockMedicalInsurance;
    ...
    <div className="p-6 max-w-[1400px] mx-auto">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="rounded-xl bg-white border border-gray-100 p-5">
                ...
                <p className="text-[10px] text-gray-400 mt-1">月缴存 ¥{insurance.monthlyDeposit}</p>
              </div>
              <div className="rounded-xl bg-white border border-gray-100 p-5">
                <p className="text-xs text-gray-500 mb-1">异地就医备案</p>
                <div className="flex items-center gap-2 mt-1">
                  <StatusBadge status={insurance.crossRegionStatus} size="md" />
                </div>
                <p className="text-[10px] text-gray-400 mt-2">自动同步全国医保平台</p>
              </div>
              <div className="rounded-xl bg-white border border-gray-100 p-5">
                <p className="text-xs text-gray-500 mb-1">最近缴费</p>
                <p className="text-lg font-bold text-gray-900">{insurance.lastPayment}</p>
            ...
            <div className="rounded-xl bg-white border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                {insurance.crossRegionRecords.map((record, i) => (
      ...
      {tab === "elderly" && (
        <div className="space-y-6">
          {mockSubsidies.filter((s) => s.type === "elderly").map((subsidy) => (
      ...
      {tab === "newborn" && (
        <div className="space-y-6">
          {mockSubsidies.filter((s) => s.type === "newborn").map((subsidy) => (
            <div key={subsidy.id} className="bg-white rounded-xl border border-gray-100 p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-4">
                {subsidy.timeline.slice(0, 4).map((step, i) => (
