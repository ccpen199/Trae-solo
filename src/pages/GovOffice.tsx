...
import { mockDocuments, mockMeetings, mockTasks } from "@/data/mock";
import StatusBadge from "@/components/StatusBadge";
import { FileText, Users, ClipboardList, Sparkles, Clock, AlertTriangle, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex gap-2 mb-6">
        {[
          { key: "documents" as Tab, label: "公文智能摘要", icon: FileText, count: mockDocuments.length },
          { key: "meetings" as Tab, label: "会议纪要生成", icon: Users, count: mockMeetings.length },
          { key: "tasks" as Tab, label: "任务督办看板", icon: ClipboardList, count: mockTasks.length },
        ].map((item) => (
          <button
            key={item.key}
      ...
      {tab === "documents" && (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
      ...
      {tab === "tasks" && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {(["todo", "in_progress", "done"] as const).map((status) => {
            const statusTasks = mockTasks.filter((t) => t.status === status);
