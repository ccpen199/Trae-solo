...
import type { InterviewOrder, Worker } from '@shared/types';
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (ordersRes.success && ordersRes.data) {
          ...
          const historyOrders: HistoryOrder[] = ordersRes.data.map(order => {
            const worker = workers.find(w => w.id === order.workerId);
            const completedAt = ['employed', 'passed', 'failed'].includes(order.status)
                                ...
                                <div className="relative pl-6 space-y-3">
                                  ...
                                  {order.timeline.map((ev, idx) => (
                                    <div key={idx} className="relative">
                                      <div className={cn('absolute -left-6 top-1 w-3.5 h-3.5 rounded-full border-2', 'bg-success-500 border-success-500')}></div>
                                      <div className="flex items-center justify-between">
                                        <span className="text-sm text-gray-900 font-medium">{ev.type}</span>
                                        <span className="text-xs text-gray-400">{new Date(ev.time).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}</span>
                                      </div>
                              ...
                              <div>
                                ...
                                <div className="space-y-3">
                                  <div className="flex justify-between py-2 border-b border-gray-100">
                                    ...
                                    <span className="font-medium">{new Date(order.scheduledDate).toLocaleDateString('zh-CN')}</span>
                                  </div>
                                  <div className="flex justify-between py-2 border-b border-gray-100">
