        ...
        <div className="container relative z-10">
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="relative max-w-xl mx-auto mb-14">
              <div className="flex items-center bg-white/10 backdrop-blur-sm rounded-xl border-2 border-amber-400/50 focus-within:border-amber-400 transition-colors">
                ...
                />
                <button className="btn-primary mr-1.5 px-5 py-2 text-sm">
                  搜索
                </button>
              </div>
            </motion.div>

            <motion.div variants={fadeInUp} className="flex justify-center gap-8 md:gap-16">
              {[
                { label: "今日新增", value: 128, formatter: (v: number) => Math.round(v).toString() },
                { label: "在线门店", value: 2847, formatter: (v: number) => Math.round(v).toLocaleString() },
                { label: "成交总额", value: 3.2, formatter: (v: number) => `¥${v.toFixed(1)}亿` },
              ].map((stat) => (
