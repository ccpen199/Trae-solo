  ...
  const filteredPushes = useMemo(
    () =>
      pushes
        ...
        .filter((p) => !activePushFilter?.subscriptionId || p.subscriptionId === activePushFilter.subscriptionId)
        .sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime()),
    [pushes, riskFilter, search, activePushFilter]
  );

  const activeFilterSubscription = useMemo(() => {
    if (!activePushFilter?.subscriptionId) return null;
    return subscriptions.find((s) => s.id === activePushFilter.subscriptionId) ?? null;
  }, [activePushFilter, subscriptions]);
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-7 space-y-4">
            <div className="space-y-3">
              ...
              {activeFilterSubscription && (
                <div className="flex items-center justify-between rounded-lg border border-gold-500/20 bg-gold-500/5 px-4 py-2.5 animate-fade-in-up">
                  <div className="flex items-center gap-2">
                    <Filter className="h-3.5 w-3.5 text-gold-400" />
                    <span className="text-xs text-slate-400">当前筛选订阅：</span>
                    <span className="text-xs font-medium text-gold-300">
                      {activeFilterSubscription.targetName}
                      <span className="ml-1 text-[10px] text-slate-500">
                        ({typeLabel[activeFilterSubscription.type]})
                      </span>
                    </span>
                    <span className="text-[10px] text-slate-500">
                      共 {filteredPushes.length} 条相关推送
                    </span>
                  </div>
                  <button
                    onClick={() => setActivePushFilter(null)}
                    className="flex items-center gap-1 rounded-md bg-finance-800/60 px-2 py-1 text-[11px] text-slate-400 transition hover:bg-finance-700/60 hover:text-slate-200"
                  >
                    <X className="h-3 w-3" />
                    清除筛选
                  </button>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between text-[11px] text-slate-500">
                    ...
                    <button
                      onClick={() => {
                        setActivePushFilter({ subscriptionId: s.id });
                        setTab('pushes');
                      }}
