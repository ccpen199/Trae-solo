...
export const useGlobalStore = create<GlobalState>((set, get) => ({
  ...
  updateNurse: (id, nurse) =>
    set((state) => ({
      nurses: state.nurses.map((n) => (n.id === id ? { ...n, ...nurse } : n)),
    })),
  removeNurse: (id) =>
  ...
  getOpenRiskTickets: () =>
    get().riskTickets.filter((t) => t.status === 'open' || t.status === 'investigating'),
  ...
  addRiskAssessment: (assessment) =>
    set((state) => ({ riskAssessments: [...state.riskAssessments, assessment] })),
