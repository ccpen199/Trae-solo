  ...
  updateNurse: (id, updates) =>
    set((state) => {
      ...
      if (
        ...
        const existingHistory = Array.isArray(currentNurse.verifyHistory) ? currentNurse.verifyHistory : [];
        const newHistoryItem: VerifyHistoryItem = {
          ...
          remark: updates.verifyResult?.manualRemark ??
            (updates.verifyStatus === 'verified'
              ? '资料齐全，审核通过，准予开展居家护理服务'
              : '审核未通过，请补充或修正相关资料'),
          time: now,
        };
        verifyHistory = [...existingHistory, newHistoryItem];
      }
  ...
  searchNurses: (keyword) => {
    ...
    return get().nurses.filter(
      (n) =>
        ...
        n.organizationName.toLowerCase().includes(lower)
    );
  },
  ...
  searchOrders: (keyword) => {
    ...
    return get().orders.filter(
      (o) =>
        ...
        (o.nurseInfo?.name?.toLowerCase() || '').includes(lower)
    );
  },
