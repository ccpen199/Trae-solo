...
FRONTEND_PORT="${FRONTEND_PORT:-49062}"
BACKEND_PORT="${BACKEND_PORT:-59062}"
...
stop_project_listener() {
  ...
  case "$cwd" in
    "$PROJECT_DIR"*) kill "$pid" 2>/dev/null || true; sleep 2 ;;
    *) echo "skip $label: port belongs to another project"; exit 1 ;;
  esac
}

echo "=== Starting may-89062 full stack ==="
echo "Frontend port: $FRONTEND_PORT"
echo "Backend port: $BACKEND_PORT"

stop_project_listener "$FRONTEND_PORT" "frontend"
stop_project_listener "$BACKEND_PORT" "backend"
if command -v screen >/dev/null 2>&1; then
  screen -S may-89062-backend -X quit >/dev/null 2>&1 || true
  screen -S may-89062-frontend -X quit >/dev/null 2>&1 || true
fi
...
if command -v screen >/dev/null 2>&1; then
  screen -dmS may-89062-backend bash -lc 'cd "$1"; exec env FRONTEND_PORT="$2" BACKEND_PORT="$3" NODE_ENV=development "$4" ./node_modules/tsx/dist/cli.mjs api/server.ts > backend.log 2>&1' _ "$PROJECT_DIR" "$FRONTEND_PORT" "$BACKEND_PORT" "$NODE_BIN"
  screen -dmS may-89062-frontend bash -lc 'cd "$1"; exec env FRONTEND_PORT="$2" BACKEND_PORT="$3" NODE_ENV=development "$4" ./node_modules/vite/bin/vite.js --host 127.0.0.1 --port "$2" --strictPort > frontend.log 2>&1' _ "$PROJECT_DIR" "$FRONTEND_PORT" "$BACKEND_PORT" "$NODE_BIN"
else
