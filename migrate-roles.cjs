...
try {
  ...
  console.log('\n5. 验证结果:')
  const users = db.prepare('SELECT id, username, name, role, cert_status FROM users ORDER BY id').all()
  for (const u of users) {
    console.log(`   ${u.id.toString().padStart(2)} ${u.username.padEnd(10)} ${u.name.padEnd(10)} ${u.role.padEnd(10)} ${u.cert_status}`)
  }
