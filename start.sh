...
start_detached "may-89063-backend" "$BACKEND_DIR" "$PROJECT_DIR/backend.log" "$PROJECT_DIR/backend.err.log" env PATH="$(dirname "$NODE_BIN"):$PATH" FRONTEND_PORT="$FRONTEND_PORT" BACKEND_PORT="$BACKEND_PORT" "$NODE_BIN" server.js

sleep 3

echo "Starting frontend on http://127.0.0.1:$FRONTEND_PORT"
start_detached "may-89063-frontend" "$FRONTEND_DIR" "$PROJECT_DIR/frontend.log" "$PROJECT_DIR/frontend.err.log" env PATH="$(dirname "$NODE_BIN"):$PATH" FRONTEND_PORT="$FRONTEND_PORT" BACKEND_PORT="$BACKEND_PORT" "$FRONTEND_DIR/node_modules/.bin/vite" --host 127.0.0.1 --port "$FRONTEND_PORT" --strictPort
