...
if [ -z "$NEXT_FRONTEND_PORT" ] || [ -z "$NEXT_BACKEND_PORT" ]; then
  echo "No available port slot for project may-63456" >&2
  exit 1
fi
...
if [ "$NEXT_FRONTEND_PORT" != "$FRONTEND_PORT" ] || [ "$NEXT_BACKEND_PORT" != "$BACKEND_PORT" ]; then
  ...
  /usr/bin/sed -i '' "s|^FRONTEND_URL=.*|FRONTEND_URL=http://127.0.0.1:$NEXT_FRONTEND_PORT|" "$PROJECT_DIR/.env"
  /usr/bin/sed -i '' "s|^BACKEND_URL=.*|BACKEND_URL=http://127.0.0.1:$NEXT_BACKEND_PORT|" "$PROJECT_DIR/.env"
...
nohup node server.cjs > "$PROJECT_DIR/frontend.log" 2>&1 < /dev/null &
frontend_pid=$!
