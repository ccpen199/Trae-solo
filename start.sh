...
tail4="$(basename "$PROJECT_DIR" | sed -E 's/.*-([0-9]+)$/\1/' | awk '{printf "%04d", substr($0, length($0)-3)}')"
tail_num=$((10#$tail4))
...
write_env_ports() {
  ...
  awk -v fp="$frontend_port" -v bp="$backend_port" '
    BEGIN { seen_frontend = 0; seen_backend = 0 }
...
start_detached "may-89043-backend" "$PROJECT_DIR" "$PROJECT_DIR/backend.log" "$PROJECT_DIR/backend.err.log" \
  env NODE_OPTIONS="${NODE_OPTIONS:+$NODE_OPTIONS }--no-warnings" "$NODE_BIN" "$PROJECT_DIR/node_modules/.bin/tsx" api/server.ts

sleep 3

echo "Starting frontend on http://127.0.0.1:$FRONTEND_PORT"
start_detached "may-89043-frontend" "$PROJECT_DIR" "$PROJECT_DIR/frontend.log" "$PROJECT_DIR/frontend.err.log" \
  "$NODE_BIN" "$PROJECT_DIR/node_modules/.bin/vite" --host 127.0.0.1 --port "$FRONTEND_PORT" --strictPort
