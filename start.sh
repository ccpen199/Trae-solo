...
if [ -z "$NEXT_FRONTEND_PORT" ] || [ -z "$NEXT_BACKEND_PORT" ]; then
  echo "No available port slot for project may-63452" >&2
  exit 1
fi
...
nohup "$PROJECT_DIR/node_modules/.bin/tsx" api/server.ts > "$PROJECT_DIR/backend.log" 2>&1 < /dev/null &
echo $! > "$PROJECT_DIR/backend.pid"

echo "Starting frontend on http://127.0.0.1:$FRONTEND_PORT"
nohup "$PROJECT_DIR/node_modules/.bin/vite" --host 127.0.0.1 --port "$FRONTEND_PORT" --strictPort --clearScreen false > "$PROJECT_DIR/frontend.log" 2>&1 < /dev/null &
