...
start_detached() {
  ...
  if command -v screen >/dev/null 2>&1; then
    ...
    (cd "$cwd" && setsid nohup "$@" > "$log" 2>&1 < /dev/null &)
  fi
}
...
for pid in $(pgrep -f 'node|npm|vite|tsx|nodemon' 2>/dev/null || true); do
...
start_detached "may-89052-backend" "$PROJECT_DIR" "$PROJECT_DIR/backend.log" "$NODE_BIN" --import tsx/esm api/server.ts

sleep 3

echo "Starting frontend on http://127.0.0.1:$FRONTEND_PORT"
start_detached "may-89052-frontend" "$PROJECT_DIR" "$PROJECT_DIR/frontend.log" "$NODE_BIN" ./node_modules/vite/bin/vite.js --host 127.0.0.1 --port "$FRONTEND_PORT" --strictPort
