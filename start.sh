...
start_detached "may-89049-backend" "$PROJECT_DIR" "$PROJECT_DIR/backend.log" "$PROJECT_DIR/backend.err.log" "$NODE_BIN" "$PROJECT_DIR/node_modules/.bin/tsx" api/server.ts

sleep 3

echo "Starting frontend on http://127.0.0.1:$FRONTEND_PORT"
start_detached "may-89049-frontend" "$PROJECT_DIR" "$PROJECT_DIR/frontend.log" "$PROJECT_DIR/frontend.err.log" "$PROJECT_DIR/node_modules/.bin/vite" --host 127.0.0.1 --port "$FRONTEND_PORT" --strictPort

sleep 5

wait_for_project_port "$BACKEND_PORT" > "$PROJECT_DIR/backend.pid"
wait_for_project_port "$FRONTEND_PORT" > "$PROJECT_DIR/frontend.pid"
curl -I --max-time 15 "http://127.0.0.1:$FRONTEND_PORT/"
