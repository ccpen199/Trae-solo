...
export function encrypt(text: string, key: string): string {
  ...
  const cipher = crypto.createCipheriv('aes-256-cbc', hashString(key).substring(0, 32), iv);
...
export function decrypt(encryptedText: string, key: string): string {
  ...
  const decipher = crypto.createDecipheriv('aes-256-cbc', hashString(key).substring(0, 32), iv);
