...
export function buildKnowledgeGraph(policies: Policy[]): Map<string, KnowledgeNode> {
...
export function searchKnowledgeGraph(
  graph: Map<string, KnowledgeNode>,
  query: string,
...
export function generateQAMatches(
  qaPairs: KnowledgeQAPair[],
  query: string,
  threshold: number = 0.5
): (KnowledgeQAPair & { score: number })[] {
  const keywords = extractKeywords(query);
...
export function matchPoliciesToProfile(
  policies: Policy[],
  profile: CitizenProfile
): (Policy & { matchScore: number; matchReasons: string[] })[] {
  const results: (Policy & { matchScore: number; matchReasons: string[] })[] = [];
