...
const PlatformDashboard: React.FC = () => {
  ...
  const quickActions = [
    { icon: <PlusOutlined />, title: '发布办件', onClick: () => navigate('/platform/tasks/publish') },
    { icon: <DashboardOutlined />, title: '查看办件', onClick: () => navigate('/platform/tasks') },
    { icon: <ClockCircleOutlined />, title: '待评审', onClick: () => navigate('/platform/tasks?status=submitted') },
    { icon: <DollarOutlined />, title: '财务中心', onClick: () => navigate('/platform/finance') }
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">平台运营工作台</h2>
          <p className="text-gray-500 mt-1">欢迎回来，查看您的办件进度</p>
        </div>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/platform/tasks/publish')}>
          发布新办件
        </Button>
      </div>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card title="办件分类分布" className="card-hover">
            {statusData.length > 0 ? (
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card
            ...
            extra={<Button type="link" onClick={() => navigate('/platform/tasks')}>查看全部 <RightOutlined /></Button>}
          >
            <Row gutter={[12, 12]}>
