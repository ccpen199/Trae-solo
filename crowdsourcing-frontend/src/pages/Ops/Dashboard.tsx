    ...
    <div>
      <Card className="mb-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-1">运维工作台</h2>
            <p className="text-gray-500">欢迎回来，常州市公共服务聚合平台查看您的工作进度和收入统计</p>
          </div>
          <div className="flex gap-3">
            <Button type="primary" onClick={() => navigate('/ops/tasks')}>
              查看全部办件
            </Button>
            <Button onClick={() => navigate('/ops/submission')}>
              提交稿件
            </Button>
          </div>
        </div>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={12} md={6}>
            <Card className="relative overflow-hidden">
              <Statistic
                title={<span className="text-gray-500">进行中办件</span>}
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={16}>
          <Card
            title="我的办件"
            extra={<Button type="link" onClick={() => navigate('/ops/tasks')}>查看全部 <ArrowRightOutlined /></Button>}
          >
            {loading ? (
              ...
              <List
                ...
                renderItem={(item) => (
                  <List.Item
                    ...
                    onClick={() => navigate(`/ops/tasks/${(item as any).task?.id}`)}
                  >
