  ...
  const taskColumns = [
    {
      ...
      render: (text: string, record: Task) => (
        <div className="cursor-pointer hover:text-primary-700" onClick={() => navigate(`/ops/tasks/${record.id}`)}>
    ...
    },
    {
      ...
      render: (_: any, record: Task) => (
        <Space>
          <Button type="link" icon={<EyeOutlined />} onClick={() => navigate(`/ops/tasks/${record.id}`)}>
            详情
          </Button>
          ...
          {record.status === 'in_progress' && (
            <Button type="primary" onClick={() => navigate('/ops/submission')}>
              提交稿件
            </Button>
          )}
        </Space>
  ...
  const bidColumns = [
    {
      ...
      render: (text: string, record: Bid) => (
        <div className="cursor-pointer hover:text-primary-700" onClick={() => navigate(`/ops/tasks/${record.taskId}`)}>
          <div className="font-medium">{text}</div>
    ...
    },
    {
      ...
      render: (_: any, record: Bid) => (
        <Space>
          <Button type="link" onClick={() => navigate(`/ops/tasks/${record.taskId}`)}>
            查看
          </Button>
    ...
    <div>
      <Card className="mb-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-800">办件管理</h2>
        </div>
      </Card>
