...
const skillOptions = [
  'UI设计', '平面设计', 'Logo设计', '插画', 'React', 'Vue', 'Node.js', 'Python',
  'Java', 'iOS', 'Android', '文案策划', '内容创作', 'SEO', '社交媒体', '视频剪辑',
  '3D建模', '动画', '品牌策划', '法律咨询', '财务咨询'
];
  ...
  const columns = [
    {
      ...
      render: (text: string, record: Provider) => (
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate(`/employer/talent/${record.id}`)}>
    ...
    },
    {
      ...
      render: (rate: number) => (
        <div>
          ...
          <div className="text-xs text-gray-500">平均响应 {record.responseTime} 分钟</div>
        </div>
      ),
    ...
    },
    {
      ...
      render: (_: any, record: Provider) => (
        <Space>
          <Button type="primary" onClick={() => navigate(`/employer/talent/${record.id}`)}>
            查看详情
          </Button>
          <Button>
            邀请合作
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {providers.map(provider => (
              <ProviderCard
                key={provider.id}
                provider={provider}
                onClick={() => navigate(`/employer/talent/${provider.id}`)}
              />
            ))}
          </div>
        ) : (
