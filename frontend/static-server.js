...
const port = Number(process.env.FRONTEND_PORT || 49095);
const backendPort = Number(process.env.BACKEND_PORT || port + 10000);
const backendUrl = process.env.BACKEND_URL || `http://127.0.0.1:${backendPort}`;
const apiBaseUrl = process.env.API_BASE_URL || `${backendUrl}/api`;
...
const envJsContent = `window.__ENV__ = {
  ...
  PROJECT_NAME: '${process.env.PROJECT_NAME || 'may-89095'}'
};
`;

fs.writeFileSync(path.join(root, 'env.js'), envJsContent, 'utf8');
