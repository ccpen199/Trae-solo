...
import { DeleteOutlined, BookOutlined, CloudOutlined } from '@ant-design/icons'
...
const CloudLibrary = () => {
  ...
  const fetchBooks = async () => {
    try {
      setLoading(true)
      const data = await userApi.getCloudLibrary({ pageSize: 50 })
      setBooks(data?.list || [])
    } catch (error) {
      console.error('获取云书馆失败:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleRemove = async (bookId, e) => {
    e.stopPropagation()
    try {
      await userApi.removeFromCloudLibrary(bookId)
      setBooks(prev => prev.filter(b => b.book_id !== bookId))
      message.success('已从云书馆移除')
    } catch (error) {
  ...
  const handleRead = (book) => {
    navigate(`/reader/${book.book_id}/0`)
  }
  ...
  if (!isLoggedIn) {
    return (
      <div style={{
        ...
        <PageEmpty description="登录后查看您的云书馆" />
  ...
  return (
    <div className="page-content" style={{ paddingBottom: 100 }}>
      ...
      <List
        dataSource={books}
        locale={{ emptyText: <PageEmpty description="云书馆为空，去添加一些书籍吧" /> }}
        renderItem={item => (
          <Card
            size="small"
