...
import { Card, Descriptions, Tag, Table, Button, Spin, Empty, Modal, Form, Select, message, Input, Space } from 'antd';
...
function ApprovalApplicationDetail() {
  const { id } = useParams();
  ...
  const handleFinalApproval = async (values) => {
    try {
      ...
      await applicationApi.finalApproval(id, {
  ...
  const canApprove = app.status === 'pending_approval';

  const scheduleColumns = [
    { title: '期数', dataIndex: 'period', key: 'period', width: 80 },
    { title: '到期日期', dataIndex: 'due_date', key: 'due_date', render: (text) => formatDateTime(text) },
    { title: '本金', dataIndex: 'principal_amount', key: 'principal_amount', render: (val) => formatCurrency(val) },
    { title: '利息', dataIndex: 'interest_amount', key: 'interest_amount', render: (val) => formatCurrency(val) },
    { title: '应还金额', dataIndex: 'total_amount', key: 'total_amount', render: (val) => formatCurrency(val) },
    ...
    <div>
      ...
      <Card title="基本信息" style={{ marginBottom: 16 }}>
        <Descriptions bordered column={2}>
          ...
          <Descriptions.Item label="客户经理">{app.manager_name || '-'}</Descriptions.Item>
          <Descriptions.Item label="风控专家">{app.risk_expert_name || '-'}</Descriptions.Item>
      ...
      {detail.repaymentSchedules?.length > 0 && (
        <Card title="还款计划" style={{ marginBottom: 16 }}>
          <Table
            dataSource={detail.repaymentSchedules}
            rowKey="id"
            pagination={false}
...
export default ApprovalApplicationDetail;
