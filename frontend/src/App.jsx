...
function HomePage({ currentCity, showToast }) {
...
function MovieDetailPage({ currentCity, showToast }) {
  const navigate = useNavigate()
...
function CinemaPage({ currentCity }) {
  const navigate = useNavigate()
  const cinemas = [
  ...
  return (
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ padding: '16px' }}>
        {cinemas.map(cinema => (
          <div key={cinema.id} style={{ background: 'white', borderRadius: '12px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', cursor: 'pointer' }} onClick={() => navigate(`/cinema/${cinema.id}`)}>
...
function CinemaDetailPage({ currentCity }) {
  const navigate = useNavigate()
  return (
    <div style={{ padding: '16px', paddingBottom: '80px' }}>
      <h1>影院详情</h1>
      <button onClick={() => navigate(-1)} style={{ marginTop: '20px', padding: '12px 24px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>返回</button>
    </div>
  )
}

function UserPage({ isLoggedIn, onLogout }) {
  const navigate = useNavigate()
    ...
    return (
      <div style={{ padding: '80px 20px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <button onClick={() => navigate('/login')} style={{ padding: '12px 32px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '16px' }}>登录</button>
          <button onClick={() => navigate('/register')} style={{ padding: '12px 32px', background: 'white', color: '#e74c3c', border: '1px solid #e74c3c', borderRadius: '8px', cursor: 'pointer', fontSize: '16px' }}>注册</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ background: 'white', margin: '16px', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px', borderBottom: '1px solid #f5f5f5', cursor: 'pointer' }} onClick={() => navigate('/favorites')}>
          <span style={{ fontSize: '20px', marginRight: '12px' }}>❤️</span>
          <span>我的收藏</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', padding: '16px', cursor: 'pointer' }} onClick={() => navigate('/settings')}>
          <span style={{ fontSize: '20px', marginRight: '12px' }}>⚙️</span>
          <span>设置</span>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', padding: '16px' }}>
        <button onClick={onLogout} style={{ padding: '12px 32px', background: 'white', color: '#e74c3c', border: '1px solid #e74c3c', borderRadius: '8px', cursor: 'pointer' }}>退出登录</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '60px 32px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: '400px', margin: '0 auto' }}>
        ...
        <button type="submit" style={{ width: '100%', padding: '14px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '8px', fontSize: '16px', cursor: 'pointer' }}>
          登录
        </button>
      </form>
      ...
      <div style={{ textAlign: 'center', marginTop: '32px' }}>
        <button onClick={() => navigate('/')} style={{ padding: '8px 24px', background: 'white', border: '1px solid #ddd', borderRadius: '8px', cursor: 'pointer' }}>稍后再说</button>
      </div>
    </div>
  )
}
...
function CitySelectPage({ currentCity, onCityChange }) {
  const navigate = useNavigate()
  const hotCities = [
    { id: 1, name: '北京' },
    { id: 2, name: '上海' },
    ...
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
          {hotCities.map(city => (
            <div key={city.id} style={{ padding: '16px', textAlign: 'center', background: '#f5f5f5', borderRadius: '8px', cursor: 'pointer' }} onClick={() => handleSelect(city)}>
              {city.name}
            </div>
          ))}
        </div>
    ...
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ background: 'white', margin: '16px', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        {['消息通知', '关于我们', '清理缓存'].map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', borderBottom: i < 2 ? '1px solid #f5f5f5' : 'none', cursor: 'pointer' }}>
            <span>{item}</span>
  ...
  return (
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ padding: '80px 20px', textAlign: 'center' }}>
        ...
        <button onClick={() => navigate('/')} style={{ marginTop: '24px', padding: '10px 32px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>去发现好电影</button>
      </div>
    </div>
  )
}
...
function AppContent() {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '32px' }}>
          <button onClick={() => { localStorage.setItem('hasSeenOnboarding', 'true'); setShowOnboarding(false) }} style={{ width: '100%', padding: '14px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '8px', fontSize: '16px', cursor: 'pointer' }}>开始体验</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      {toast && <div style={{ position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)', padding: '12px 24px', background: '#333', color: 'white', borderRadius: '8px', zIndex: 1000 }}>{toast}</div>}
      <Routes>
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        <Route path="/movie/:id" element={<MovieDetailPage currentCity={currentCity} />} />
        <Route path="/cinemas" element={<CinemaPage currentCity={currentCity} />} />
        <Route path="/cinema/:id" element={<CinemaDetailPage currentCity={currentCity} />} />
        <Route path="/user" element={<UserPage isLoggedIn={isLoggedIn} onLogout={handleLogout} />} />
        <Route path="/login" element={<LoginPage onLogin={handleLogin} />} />
