    ...
    <el-card>
      <template #header>
        <div class="card-toolbar">
          <span class="card-title">装船管理</span>
        </div>
      </template>
      ...
      <el-table :data="bookings" stripe v-loading="loading">
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" link @click="handleLoading(row)">
              执行装船
            </el-button>
          </template>
        </el-table-column>
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const result = await bookingsApi.loading(currentBooking.value.id, {
          comment: form.comment,
        })
        if (result.success) {
          ElMessage.success('装船操作成功')
        ...
        console.error('Loading action error:', error)
