...
function isTalentScenario() {
  return scenario.title.includes("山东区域垂直招聘") || frontendPort === 49094;
}
...
function applyScenarioCopy() {
  ...
  if (isTalentScenario()) {
    document.querySelector("#purchase .eyebrow").textContent = "岗位流程 / 提交";
    document.querySelector("#purchase .muted").textContent = "提交后生成岗位匹配、简历建档、企业资质核验、面试预约和录用意向状态，写入 SQLite 后个人中心与后台同步刷新。";
    $("#orderSubmit").textContent = "提交岗位匹配流程";
  }
}
...
function serviceCard(item) {
  if (isTalentScenario()) {
    return `
      <article class="card talent-card">
        <div class="card-top">
        ...
        <div class="meta">
          ...
          <span>匹配评分 ${item.rating}</span>
        </div>
        <div class="card-actions">
          <strong>流程状态：可建档 / 可预约</strong>
          <button type="button" data-buy="${item.id}">发起匹配</button>
        </div>
      </article>
    `;
  }
...
function renderServices(items, searchTerm = "") {
  ...
  $("#serviceSelect").innerHTML = items.length
    ? items.map((item) => `<option value="${item.id}">${isTalentScenario() ? "流程：" : ""}${item.title}${isTalentScenario() ? " / 建档匹配" : ` - ¥${item.price}`}</option>`).join("")
...
async function loadProfile() {
  ...
  $("#profileBox").innerHTML = `
    ...
    <div><span class="label">${isTalentScenario() ? "岗位匹配流程" : "订单"}</span><strong>${profile.data.stats.orders}</strong></div>
    <div><span class="label">${isTalentScenario() ? "资质/补贴需求" : "需求"}</span><strong>${profile.data.stats.submissions}</strong></div>
  `;
  $("#ordersBox").innerHTML = recentOrders.length
    ? recentOrders.map((order) => `<div class="row"><span>#${order.id} ${order.customer} ${compactText(order.note)}</span><strong>${order.status}${isTalentScenario() ? " / 面试协同待跟进" : ` ¥${order.amount}`}</strong></div>`).join("")
...
async function loadAdmin() {
  ...
  `;
  if (isTalentScenario()) {
    const heatmap = data.talentHeatmap || [];
    const metrics = data.recruitmentMetrics || {};
    const campuses = data.campusCooperation || [];
    const audits = data.auditChecks || [];
    $("#adminTalentPanel").innerHTML = `
      <article>
        <h3>区域人才供需热力图</h3>
        ${heatmap.map((h) => `<div class="row"><span>${h.city} · ${h.industry}</span><strong>岗位${h.jobs} / 人才${h.talents} / 缺口${h.gap}</strong></div>`).join("")}
      </article>
      <article>
        <h3>招聘效能指标</h3>
        <div class="row"><span>简历转化率</span><strong>${metrics.resumeConversion || 0}%</strong></div>
        <div class="row"><span>面试到场率</span><strong>${metrics.interviewAttendance || 0}%</strong></div>
        <div class="row"><span>Offer接受率</span><strong>${metrics.offerAcceptance || 0}%</strong></div>
        <div class="row"><span>虚假职位拦截</span><strong>${metrics.fakeJobBlocked || 0}条</strong></div>
      </article>
      <article>
        <h3>高校合作进展</h3>
        ${campuses.map((c) => `<div class="row"><span>${c.school}</span><strong>${c.majorMatch}% 专业匹配 / ${c.retention}% 留存</strong></div>`).join("")}
      </article>
      <article>
        <h3>复查审计视图</h3>
        ${audits.map((a) => `<div class="row"><span>${a.item}</span><strong>${a.status} · ${a.result}</strong></div>`).join("")}
      </article>
    `;
  }
...
function bindEvents() {
  ...
  $("#orderForm").addEventListener("submit", async (event) => {
    ...
    showResult(isTalentScenario() ? { ...data, flow: "已生成简历建档、LBS岗位匹配、企业资质核验、面试预约、录用意向跟进状态" } : data);
